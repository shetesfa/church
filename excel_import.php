<?php
// excel_export.php - Export with options, logo, beautiful formatting and category support
...
$category = $_POST['category'] ?? $_GET['category'] ?? 'all'; // NEW: category filter
...
$query .= " ORDER BY category ASC, year DESC, full_name ASC"; // Added category to order
...
// Headers - Added Category column
$headers = [
    'መታወቂያ ቁጥር', 'ሙሉ ስም', 'ክርስትና ስም', 'ምድብ', 'ስልክ ቁጥር', 'የትውልድ ቀን', 'ዕድሜ',
...
while($row = $result->fetch_assoc()) {
    ...
    // Add category separator row if category changes
    if($current_category != $row['category']) {
        $current_category = $row['category'];
        $sheet->insertNewRowBefore($rowNum, 1);
        $categoryRow = $rowNum;
        
        $categoryLabel = ($current_category == 'ህጻናት') ? '🧒 ህጻናት' : '👦 ወጣቶች';
        $sheet->setCellValue('A' . $categoryRow, $categoryLabel);
        $sheet->mergeCells('A' . $categoryRow . ':W' . $categoryRow);
        $sheet->getStyle('A' . $categoryRow)->getFill()
              ->setFillType(Fill::FILL_SOLID)
              ->getStartColor()->setRGB(($current_category == 'ህጻናት') ? '3498db' : '2ecc71');
        $sheet->getStyle('A' . $categoryRow)->getFont()->setBold(true)->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('FFFFFF'));
        $sheet->getStyle('A' . $categoryRow)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
    ...
    if(($rowNum - $startRow) % 2 == 0) {
        $sheet->getStyle('A' . $rowNum . ':X' . $rowNum)->getFill()
              ->setFillType(Fill::FILL_SOLID)
    ...
    // Category with emoji
    $category_display = ($row['category'] == 'ህጻናት') ? '🧒 ህጻናት' : '👦 ወጣቶች';
    $sheet->setCellValue($col++ . $rowNum, $category_display);
    ...
    $sheet->setCellValue($col++ . $rowNum, $age); // Age column
    
    $gender = $row['gender'];
    if($gender == 'ወ' || $gender == 'Male') $gender_display = 'ወንድ';
    elseif($gender == 'ሴ' || $gender == 'Female') $gender_display = 'ሴት';
    else $gender_display = '---';
    $sheet->setCellValue($col++ . $rowNum, $gender_display);
    ...
    // Highlight missing fields based on category
    if($row['category'] == 'ወጣቶች' && empty($row['phone'])) {
    ...
    if(empty($row['gender'])) {
        $sheet->getStyle('H' . $rowNum)->getFill()
              ->setFillType(Fill::FILL_SOLID)
              ->getStartColor()->setRGB('FFF3CD');
    }
...
$sheet->getStyle('A' . $startRow . ':X' . ($rowNum-1))->applyFromArray([
    'borders' => [
        'allBorders' => [
            'borderStyle' => Border::BORDER_THIN,
        ],
...
if($category != 'all') $filterInfo[] = "ምድብ: " . ($category == 'ህጻናት' ? 'ህጻናት' : 'ወጣቶች');
...
// Set filename
$filename = 'አባላት_';
if($category != 'all') $filename .= ($category == 'ህጻናት' ? 'ህጻናት' : 'ወጣቶች') . '_';
            ...
            <form method="POST" action="excel_export.php?export=1" class="export-form">
                ...
                <div class="form-group">
                    ...
                    <select name="category" class="form-control">
                        ...
                        <option value="ህጻናት">🧒 ህጻናት</option>
                        <option value="ወጣቶች">👦 ወጣቶች</option>
                    </select>
                </div>
